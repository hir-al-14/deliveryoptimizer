...
http_server_start \
  ...
  DELIVERYOPTIMIZER_SOLVER_QUEUE_WAIT_MS=200
...
second_http_code="$("${curl_bin}" -sS --max-time 5 -o "${second_response_file}" -w "%{http_code}" \
  -X POST \
  -H "Content-Type: application/json" \
  --data-binary "@${payload_file}" \
  "$(http_server_url /api/v1/deliveries/optimize)")"

if [[ "${second_http_code}" != "503" ]]; then
  echo "expected HTTP 503 when queue wait expires, got ${second_http_code}" >&2
...
if [[ "$(cat "${first_http_code_file}")" != "200" ]]; then
  echo "expected the in-flight solve to complete with HTTP 200" >&2
  cat "${first_response_file}" >&2 || true
  exit 1
fi

if [[ "$(wc -l <"${vroom_started_file}")" -ne 1 ]]; then
  echo "expected only the first solve to reach VROOM before the queued request expired" >&2
  cat "${vroom_started_file}" >&2 || true
  exit 1
fi
